...
def check_ten_yr(date):
    # Use a loop to look back for data instead of recursion
    current_date = date
    for _ in range(15): # Try looking back up to 15 * 2 = 30 days
        data = Ticker("^TNX").history(start=current_date - timedelta(days=5), end=current_date)
        if not data.empty:
            # most recent date
            return data.tail(1)["Close"].item() / 100
        # If data is empty, try looking back a couple more days
        current_date -= timedelta(days=2)
        print(f"No ^TNX data found for {current_date + timedelta(days=2)}. Trying {current_date}...")
    
    # If data is not found after looking back, return a default value or raise an error
    print("Warning: ^TNX data not found within the lookback period. Returning 0.0.")
